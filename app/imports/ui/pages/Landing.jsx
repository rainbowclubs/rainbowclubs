...
class Landing extends React.Component {
  ...
  renderUser() {
    const preferredClubs = this.props.clubs.filter(function (club) {
      return club.types.some(value => Meteor.user().profile.types.includes(value));
    });
    const randomClubs = [];
    for (let i = 0; i < 3; i++) {
      randomClubs.push(preferredClubs[Math.floor(Math.random() * preferredClubs.length)]);
    }
    ...
    return (
        <div>
          <Container>
            <Header className='UHGreenTextColor' as="h1" textAlign="center">Clubs Matching Your Interests</Header>
            <Card.Group>
